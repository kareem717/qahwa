...
- (void)dealloc {
    Log("Deallocating AudioManager", "debug");
...
- (BOOL)startCapture:(NSError **)error {
    ...
    if (status != noErr) {
        Log("Failed to create IO proc for aggregate device", "error");
        if (error) {
...
- (void)handleAudioInput:(const AudioBufferList *)bufferList {
    ...
    if (!callback) {
        Log("Skipping audio input - no callback", "debug");
        return;
    }
    ...
    @autoreleasepool {
        ...
        @try {
            ...
            if (!monoBuffer) {
                Log("Failed to convert to mono", "error");
                return;
            }
            ...
            if (!resampledBuffer) {
                Log("Failed to resample buffer", "error");
                return;
            }
            ...
            if (audioData) {
                dispatch_async(_audioQueue, ^{
                    @try {
                        ...
                        Log(std::string("Exception in callback: ") + std::string([exception.description UTF8String]), "error");
                    }
                });
            } else {
                Log("Failed to create NSData from resampled buffer", "error");
            }
        } @catch (NSException *exception) {
...
- (BOOL)setupAggregateDeviceIfNeeded:(NSError **)error {
    ...
    NSString *masterDeviceUID = (__bridge NSString *)outputUID;
